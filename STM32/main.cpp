...
void moveMotorsTask(void *argument)
{
	...
	#define WALL_DIST   170   // distância ideal à parede esquerda (mm)
	#define FRONT_STOP  120   // distância mínima frontal (mm)
	#define VEL_BASE    400   // velocidade base
	#define Kp          1.3f  // ganho proporcional
	...
	for(;;){
		...
		case 2: // modo navegação
        {
            ...
            else
            {
            	...
            	if(FrontDist > 300 ){   // andar em frente se nao tiver parede
					__HAL_TIM_SET_COMPARE(&htim15, TIM_CHANNEL_1, 400);
					__HAL_TIM_SET_COMPARE(&htim15, TIM_CHANNEL_2, 400);
