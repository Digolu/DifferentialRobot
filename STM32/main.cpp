	...
	for(;;)
	{
		if(isPacketValid(&comms_packet))
		{
			...
			case COMMS_TYPE_WRITE:
			{
				...
				char buff[100];
				uint8_t len = sprintf(buff, "FrontDist = %.2f mm; EsqDist = %.2f mm; DirDist = %.2f mm\r\n", FrontDist, EsqDist, DirDist);
...
void moveMotorsTask(void *argument)
{
	...
	#define WALL_DIST   150   // distância ideal à parede esquerda (mm)
	#define FRONT_STOP  100   // distância mínima frontal (mm)
	#define VEL_BASE    500   // velocidade base
	#define Kp          1.2f  // ganho proporcional

	for(;;){
		int modos = 1;
		if (recebido == 'm') modos = 1; // modo manual
		if (recebido == 'n') modos = 2; // nodo de navegação automatica
		...
		break;
		case 2: // modo navegação
        {
            if (FrontDist > 0 && FrontDist < FRONT_STOP)
            {
                // Obstáculo à frente — para e vira para o lado com mais espaço
                ...
                __HAL_TIM_SET_COMPARE(&htim15, TIM_CHANNEL_2, 400);

                // Roda até ter espaço à frente
                while (FrontDist > 0 && FrontDist < FRONT_STOP) {
                    osDelay(10);
                }

                // Para antes de retomar
                __HAL_TIM_SET_COMPARE(&htim15, TIM_CHANNEL_1, 0);
                __HAL_TIM_SET_COMPARE(&htim15, TIM_CHANNEL_2, 0);
                osDelay(100);
            }
            ...
            {
                // Caminho livre — controlador P para seguir parede esquerda
                float erro     = (float)(WALL_DIST - EsqDist);
                float correcao = Kp * erro;

                int32_t vel_esq = VEL_BASE + (int32_t)correcao;
                int32_t vel_dir = VEL_BASE - (int32_t)correcao;

                if (vel_esq < 100) vel_esq = 100;
                if (vel_esq > 700) vel_esq = 700;
                if (vel_dir < 100) vel_dir = 100;
                if (vel_dir > 700) vel_dir = 700;

                // Frente
                HAL_GPIO_WritePin(M1_DIR_GPIO_Port, M1_DIR_Pin, GPIO_PIN_RESET);
                HAL_GPIO_WritePin(M2_DIR_GPIO_Port, M2_DIR_Pin, GPIO_PIN_SET);

                __HAL_TIM_SET_COMPARE(&htim15, TIM_CHANNEL_1, vel_esq);
                __HAL_TIM_SET_COMPARE(&htim15, TIM_CHANNEL_2, vel_dir);
            }
            break;
        }
